    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(Intent.ACTION_MY_PACKAGE_REPLACED) || intent.getAction().equals(Intent.ACTION_BOOT_COMPLETED) || intent.getAction().equals("android.intent.action.QUICKBOOT_POWERON")) {
            ...
            if (autoStart) {
                ...
                // DURAKTA PATCH: if the start throws (e.g. a forbidden
                // foreground-service start), the lock taken above must not be
                // left pinned until process death. Release on failure.
                try {
                    ...
                    BackgroundService.releaseLockIfHeld();
                    throw e;
                }
            }
        }
