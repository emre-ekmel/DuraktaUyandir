...
public class BackgroundService extends Service implements MethodChannel.MethodCallHandler {
    ...
    public void onDestroy() {
        // DURAKTA PATCH (audit 2026-08-20 task 4): the plugin acquires a
        // reference-counted PARTIAL_WAKE_LOCK in runService() but has NO
        // release path, so the CPU stayed awake until process death. The wakelock
        // is process-scoped, so it also outlived the service itself whenever
        // the UI engine was still running. Release here — onDestroy is the one
        // convergent exit for stopSelf, system reclaim, and manual stops.
        releaseLockIfHeld();
    ...
    // DURAKTA PATCH: centralized, idempotent release used from onDestroy and
    // from the BootReceiver failure path. Never throws.
    public static void releaseLockIfHeld() {
        try {
            if (lockStatic != null && lockStatic.isHeld()) {
                lockStatic.release();
            }
        } catch (RuntimeException e) {
            Log.w(TAG, "releaseLockIfHeld: " + e.getMessage());
        }
    }
    ...
    private void runService() {
        try {
            ...
            // DURAKTA PATCH: lock is reference-counted; only take it if not
            // already held. Previously every runService() bump stacked a ref
            // that was never released (service restarts inflated the count).
            WakeLock wakeLock = getLock(getApplicationContext());
            if (!wakeLock.isHeld()) {
                wakeLock.acquire();
            }
